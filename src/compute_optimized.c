...
int32_t check[4];
if (n - idx >= 4) {
  __m128i c = _mm_loadu_si128((__m128i *) vec1 + idx);
  //c = _mm_and_si128(c, _mm_set1_epi32(0xFFFFFFFF));
  _mm_storeu_si128((__m128i *) check, c);

  __m128i d = _mm_loadu_si128((__m128i *) vec2 + (n - idx + 4));
  //d = _mm_and_si128(d, _mm_set1_epi32(0xFFFFFFFF));
  _mm_storeu_si128((__m128i *) check, d);

  __m128i reverse_d = _mm_shuffle_epi32(d, 0b11100100);
  _mm_storeu_si128((__m128i *) check, reverse_d);

  __m128i tmp = r(c, reverse_d);

  int32_t tmp_arr2[4];
  _mm_storeu_si128((__m128i *) tmp_arr2, tmp);
